...
class AssetAPIHandler(RouteHandler):
    def register_routes(self) -> None:
        self.route(
            "/api/asset/request_uploads", "asset_request_uploads", ["POST"]
        )
    ...
    async def asset_request_uploads(
        ...
        for file in payload.files:
            ...
            asset_uuid = uuid4()
            asset_key_orig = (
                self.app.asset_manager.mint_asset_key_for_presigned_slots(
                    user_id, f"{asset_uuid}{ext}"
                )
            )
            # TODO: consider batching this if it becomes a bottleneck
            asset_upload_url = (
                await self.app.asset_manager.generate_signed_url_put(
                    asset_key_orig
                )
            )
        ...
        try:
            while True:
                try:
                    ...
                    logging.info(
                        f"[WS] WebSocket disconnected for user_id={user_id}"
                    )
                    break
                except Exception as e:
                    logging.warning(
                        f"[WS] Malformed JSON from user_id={user_id}: {e}"
                    )
                    continue
                ...
                try:
                    ...
                    logging.warning(
                        f"[WS] Failed to handle msg event, exception: {e}"
                    )
                    continue
